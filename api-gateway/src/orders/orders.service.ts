...
export class OrdersService {
  ...
  async cancelOrder(userId: string, orderId: string) {
    return firstValueFrom(
      this.ordersClient.send({ cmd: 'cancel_order' }, { userId, orderId }),
    );
  }
  ...
  async getOrderById(orderId: string) {
    return firstValueFrom(
      this.ordersClient.send({ cmd: 'get_order_by_id' }, { orderId }),
    );
  }

  async refundOrder(userId: string, orderId: string) {
    return firstValueFrom(
      this.ordersClient.emit('refund_order', { userId, orderId }),
    );
  }
}
